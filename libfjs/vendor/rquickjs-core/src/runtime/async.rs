...
use super::{
    ...
    InterruptHandler, JobErrorHandler, MemoryUsage, PromiseHook, RejectionTracker,
};
...
impl AsyncRuntime {
    ...
    /// Set a closure which is called when a background JavaScript job throws.
    #[inline]
    pub async fn set_job_error_handler(&self, handler: Option<JobErrorHandler>) {
        self.inner
            .lock()
            .await
            .runtime
            .get_opaque()
            .set_job_error_handler(handler);
    }
            ...
            loop {
                ...
                match pending {
                    Err(e) => {
                        ...
                        if lock.runtime.get_opaque().has_job_error_handler() {
                            lock.runtime.get_opaque().run_job_error_handler(ctx);
                            continue;
                        }
                        let err = ctx.catch();
