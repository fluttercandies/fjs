...
pub(crate) struct Opaque<'js> {
    ...
    /// The user provided background JavaScript job error handler, if any.
    #[cfg(feature = "futures")]
    job_error_handler: UnsafeCell<Option<JobErrorHandler>>,
...
impl<'js> Opaque<'js> {
    pub fn new() -> Self {
        Opaque {
            ...
            #[cfg(feature = "futures")]
            job_error_handler: UnsafeCell::new(None),
    ...
    #[cfg(feature = "futures")]
    pub fn set_job_error_handler(&self, handler: Option<JobErrorHandler>) {
        unsafe { (*self.job_error_handler.get()) = handler }
    }

    #[cfg(feature = "futures")]
    pub fn has_job_error_handler(&self) -> bool {
        unsafe { (*self.job_error_handler.get()).is_some() }
    }

    #[cfg(feature = "futures")]
    pub fn run_job_error_handler<'a>(&self, ctx: Ctx<'a>) {
        unsafe { (*self.job_error_handler.get()).as_mut().unwrap()(ctx) }
    }
    ...
    pub fn clear(&mut self) {
        ...
        #[cfg(feature = "futures")]
        self.job_error_handler.get_mut().take();
